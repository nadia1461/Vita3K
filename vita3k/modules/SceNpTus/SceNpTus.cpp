...
// TSS/TUS calls can also emit
// - SCE_NET_CTL_ERROR_FLIGHT_MODE_ENABLED
// - SCE_NET_CTL_ERROR_WIFI_DISABLED
// error codes???
...
// A standard TSS/TUS flow follows
// - sceNpTusInit
// - sceNpTusCreateTitleCtx
// 
// Many Individual Requests
// - sceNpTusCreateRequest
// - A TUS/TSS call like sceNpTssGetSmallStorage
// - sceNpTusDeleteRequest
//
// Game Ended
// - sceNpTusDeleteTitleCtx
// - sceNpTusTerm - ensure all used, but un-deleted, request IDs are deleted

// TSS/TUS is currently mostly un-implemented, but will return abort codes during init-related calls
// This allows a more graceful handling than returning `UNIMPLEMENTED()` which is interpreted as a success code (0), while also keeping the impact low
// Returning a success during `TusInit`/`TusCreate*` would mislead games into thinking they can proceed with the TSS/TUS flow
//
// Calling an unimplemented TUS/TSS call, like `sceNpTssGetSmallStorage`, returns a success code (0) but with an uninitialized data buffer and an incorrect size
// This can result in buffer over-reads, over-writes, and parsing errors due to the garbage buffer content + garbage length returned
...
EXPORT(int, sceNpTusCreateRequest) {
    LOG_WARN_ONCE("Stubbed {} import called. ({})", export_name, "Aborting TUS Request - further processing is unimplemented");
    return SCE_NP_COMMUNITY_ERROR_ABORTED;
}
...
EXPORT(int, sceNpTusInit) {
    LOG_WARN_ONCE("Stubbed {} import called. ({})", export_name, "Aborting TUS Initialization - further processing is unimplemented");
    return SCE_NP_COMMUNITY_ERROR_ABORTED;
}
